...
function markOtpSend(key) {
    const history = cleanupOtpHistory(key);
    history.push(Date.now());
    otpSendHistory.set(key, history);
}
function getLockRemaining(state) {
    if (!state) {
        return 0;
    }
    return Math.max(0, Math.ceil((state.lockedUntil - Date.now()) / 1000));
}
function recordFailure(map, key, maxFailures, lockoutMs) {
    const now = Date.now();
...
function cleanupExpiredEnvelope(envelope) {
...
io.on("connection", (socket) => {
    ...
    socket.on("status:sync", async ({ chattedUserIds }) => {
        cleanupStatuses();
        const result = [];
    ...
    socket.on("status:create", ({ type, content, caption }) => {
        cleanupStatuses();
    ...
    socket.on("channel:create", ({ name, description, avatar }) => {
        const newChan = {
            id: `channel-${Date.now()}`,
            name,
            description,
    ...
    socket.on("channel:post", ({ channelId, content }) => {
    ...
    socket.on("community:create", ({ name, description, groups }) => {
        const newComm = {
